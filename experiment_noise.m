...
k = 8;%floor(.05*n);
% M = n-3;
Ms = 7:19; %floor([.05 .1 .15 .2 .25 .3 .35]*n);
M = length(Ms);
k_alg = 15;
% k_alg = 9;
epsilon = 0.05;
...
alg = 8;
errs_clean = zeros(alg, M);
errs_noise = zeros(alg, M);
errs_clean_unit = zeros(alg, M);
errs_noise_unit = zeros(alg, M);
timez_A = zeros(alg, M);

errs_clean_2 = zeros(alg, M);
errs_noise_2 = zeros(alg, M);
errs_clean_unit_2 = zeros(alg, M);
timez_B = zeros(alg, M);
...
for i = 1:n_avg
  ...
  for m = Ms
    ...
    errs_clean_2(8, mm) = errs_clean_2(8, mm) + stability_error(x_ind, find(abs(x_sl0) > DELTA), n);
    ...
    errs_clean_unit(8, mm) = errs_clean_unit(8, mm) + per_error(x/norm(x), x_sl0)/norm(x_sl0)));
    ...
    errs_noise_2(8, mm) = errs_noise_2(8, mm) + stability_error(x_ind, find(abs(x_sl0) > DELTA), n);
    ...
    errs_noise_unit(8, mm) = errs_noise_unit(8, mm) + per_error(x/norm(x), x_sl0/norm(x_sl0));
